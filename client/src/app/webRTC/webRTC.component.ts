...
export class WebRTCComponent implements OnInit {
  ...
  async connectPeers() {
    ...
    this.pc.onicecandidate = e => !e.candidate || this.emit('onicecandidate', {candidate : e.candidate});
    this.socket
      .fromEvent('onicecandidate')
      .subscribe((data: any) => {
        return  data.senderId === this.senderId || this.pc.addIceCandidate(data.candidate).catch(console.log);
      });
    ...
    this.socket
      .fromEvent('offer')
      .subscribe((data: any) => data.senderId === this.senderId ||
        this.pc.setRemoteDescription(data.desc).then(async () => {
    ...
    this.socket
      .fromEvent('answer')
      .subscribe((data: any) => data.senderId === this.senderId || this.pc.setRemoteDescription(data.answer));
  }
