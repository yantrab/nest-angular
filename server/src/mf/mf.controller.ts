...
import { App, User, UserFilter } from 'shared';
...
export class MFController {
    ...
    saveUserSettings(@Body() userSettings: Partial<UserSettings>) {
        return this.mfService.saveUserSettings(userSettings);
    }

    @Post('saveUserFilters')
    saveUserFilters(@Body() userFilter: UserFilter[], @ReqUser() user: User) {
        return this.mfService.saveUserFilters(userFilter, user.email);
    }
}
