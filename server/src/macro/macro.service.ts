...
export class MacroService {
  ...
  constructor(private repositoryFactory: RepositoryFactory) {
    ...
    this.seriesRepo = this.repositoryFactory.getRepository<Series>(
      Series,
      'DBMacro',
    );
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const ids = (await pool
          .request()
          .query(
            'select distinct pasp_id as id from [dbMacro].[dbo].[prData]',
  ...
  private async updateCategoriesAndSerias() {
    const getChildren = (categories, category) => {
      const children = categories.filter(
        c =>
          c._id.length === category._id.length + 1 &&
          c._id.slice(0, category._id.length) === category._id,
      );
      if (!children.length) {
        return undefined;
      }
      children.forEach(
        child => (child.children = getChildren(categories, child)),
      );
      return children;
    };
