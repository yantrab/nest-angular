...
export class MacroService {
    ...
    constructor(private repositoryFactory: RepositoryFactory) {
        ...
        // this.updateCategoriesAndSeries().then(x => console.log('blabla'));
    }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            `)).recordset.map(s => {
                    const serie: Series = new Series({
                        ...
                        isStopped: s.source === 'Stopped',
                        hebTypeName: s.hebTypeName,
                        sId: s.sId,
                        name: s.name,
                        unitEnName: s.unitEnName,
                    });

                    for (let i = 1; i <= 3; i++) {
                        const subId = serie.sId.slice(0, i);
                        serie.catalogPath += categoriesDB.find(c => c.cId === subId).name;
    ...
    async getUserSettings(email: string): Promise<UserSettings> {
        let userSettings = await this.userSettingsRepo.findOne({ email });
        if (!userSettings) {
            userSettings = new UserSettings({ email, userTemplates: [{ seriesIds: [], name: 'טמפלט' }] });
            const savedResult = (await this.saveUserSettings(userSettings)) as InsertOneWriteOpResult;
            userSettings._id = savedResult.insertedId;
        }
        return userSettings;
    }
