...
export class MFService {
            ...
            if (!data) {
                this.mfSettingsRepo.saveOrUpdateOne({
                    ...
                    simlulationSettings: {
                        ...
                        customizeParameters: [
                            new CustomizeParameterGroup({ name: 'exp', parameters: exp.map(e => ({ name: e, path: e })) }),
                            new CustomizeParameterGroup({ name: 'yields', parameters: yileds.map(e => ({ name: e, path: e })) }),
                            new CustomizeParameterGroup({
                                name: 'durations',
                                parameters: durations.map(e => ({ name: e, path: e })),
                            }),
                            new CustomizeParameterGroup({
                                name: 'Management',
                                parameters: [{ name: 'dailyYield', path: 'dailyYield' }],
                            }),
                        ],
                    },
                });
    ...
    async saveUserSettings(userSettings: Partial<UserSettings>) {
        if (userSettings.isNew) {
            return this.mfUserSettingsRepo.collection.insertOne(userSettings);
        }
    }

    saveUserFilters(userFilter: UserFilter[], email: string) {
        return this.mfUserSettingsRepo.collection.updateOne({ email: email }, { $set: { userFilters: userFilter } });
    }
}
